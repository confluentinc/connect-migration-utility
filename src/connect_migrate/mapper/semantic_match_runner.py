"""
Semantic-matching helpers used by the SM->FM pipeline.

Wraps the embedding-based ``SemanticPropertyMatcher`` to (a) pick FM template
config-defs for SM configs we couldn't place by name, (b) walk the SM template
to recover the SM property definition for a given config, and (c) validate
that the resulting FM config has values for every required FM property and
that those values lie within the recommended set.
"""
...
class SemanticMatchRunner:
    def __init__(
        self,
        semantic_matcher: SemanticPropertyMatcher,
        logger: Optional[logging.Logger] = None,
    ):
        self.semantic_matcher = semantic_matcher
        self.logger = logger or logging.getLogger(__name__)

    def do_semantic_matching(
        self,
        ...
        for config_name in semantic_match_list:
            if config_name in user_configs:
                ...
                sm_prop = self.get_sm_property_from_template(config_name, sm_template)
    ...
    def get_sm_property_from_template(
        self,
        config_name: str,
        sm_template: Dict[str, Any],
