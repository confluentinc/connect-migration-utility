...
from src.offset_manager import OffsetManager
...
class ConnectorCreator:
    ...
    def create_connector_from_config(
        ...
        return self.create_connector_api_call(url, name, body, headers, disable_ssl_verify)
    ...
    def create_connector_from_json_file(
        ...
        for entry in connector_entries:
            ...
            self.logger.info(f"[INFO] Request body for connector '{name}': {json.dumps(redacted_body, indent=2)}")
            response = self.create_connector_api_call(url, name, body, headers, disable_ssl_verify)
...
def main():
    ...
    #TODO: get to know the default value for disable_ssl_verify
    ...
    if migration_mode=='create':
        for json_file in fm_config_dir.glob("*.json"):
            ...
            try:
                ...
                for conn in created_connectors:
                    print(f"Created connector: {conn.get('name')} with ID: {conn.get('id')}")
            except Exception as e:
        ...
        for config in connector_fm_configs:
            fm_entry = connector_fm_configs[config]
            fm_name = fm_entry['name']
            fm_config = fm_entry['config']
            fm_worker = fm_entry.get('worker', None)

            # Find matching connector from workers by name
            matching_worker_config = next((wc for wc in worker_connector_configs if wc['name'] == fm_name), None)
            if not matching_worker_config:
                logger.warning(f"No matching connector found on workers for FM connector '{fm_name}'")
                continue

            worker_offsets = matching_worker_config.get('offsets', None)
            if not worker_offsets:
                ...
                fm_config['offsets'] = worker_offsets
            ...
            try:
                ...
                #stop connector
                if config_validation and migration_mode=='stop_create_latest_offset':
                    logger.info("Stopping CP connector after validation as per migration mode")
                    #TODO: Handle disable_ssl_verify
                    creator.stop_cp_connector(fm_worker, fm_name, disable_ssl_verify)


                logger.info(f"Creating connector '{fm_name}' with offsets from workers")
                created_connectors = creator.create_connector_from_config(
                    environment_id=env_id,
                    kafka_cluster_id=lkc_id,
                    kafka_api_key=kafka_api_key,
                    kafka_api_secret=kafka_api_secret,
                ...
                for conn in created_connectors:
                    logger.info(f"Created connector: {conn.get('name')} with ID: {conn.get('id')}")
            except Exception as e:
