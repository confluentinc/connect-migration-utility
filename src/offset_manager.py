...
class OffsetManager:
    ...
    def getOffsetsOfConnector(self, discovery: ConfigDiscovery, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        if not self.is_offset_supported_connector(config['type'], config['config'].get('connector.class', '')):
        ...
        try:
            offsets = discovery._get_json_from_url(offsets_url)
    ...
    def get_connector_configs_offsets(self, discovery: ConfigDiscovery, worker_urls: List[str]) -> List[Dict[str, Any]]:
        ...
        for worker_url in worker_urls:
            ...
            configs_with_offsets.extend(discovery._get_connector_configs(worker_url))

        # Get offsets for each connector
        for config in configs_with_offsets:
            offsets = self.getOffsetsOfConnector(discovery, config)
            if offsets:
